...
For this initial work, we evaluated some CSL code on a virtualized $3\times3$ PE array emulated with conventional CPU hardware, while other experiments used hundreds of thousands of PEs a physical CS-2 device.
...
Each slot in the next generation is populated the highest-fitness genome among $n=5$ randomly sampled individuals, ties broken randomly.
...
At the outset of simulation, the first 16 bits of founding genomes were randomized and, subsequently, were inherited without mutation, thus identifing descendants of the same founding ancestor.

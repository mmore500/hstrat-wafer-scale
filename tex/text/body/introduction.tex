...
Crucially, however, the scale of population size is core to key directions in artificial life research, like transitions in individuality, ecological dynamics, and rare evolutionary innovations \citep{CITETODO}.
...
In existing artificial life work, a typical approach uses centralized tracking to maintain an exact, complete record comprising all parent-offspring relationships that have existed over the course of a simulation \citep{phylotrackandeverythingphylotrackcitedTODO}.
Often, records of extinct lineages are pruned to prevent memory bloat \citep{TODOCITEPREPRINT}.
...
Although analogous work with natural biosequences is notoriously challenging, uncertain, biased, and data-intensive \citep{TODOthatcitationfromabsractofpreprint},
...
Designed for attachment to underlying replicators as a neutral annotation (akin to noncoding DNA), it is a general-purpose technique potentially applicable across study domains \citep{FUNCITATIonsFRomPREPRINT}.

...
One defining characteristic of computational artificial life experiments is the near total malleability of the simulacrum \citep{pattee1989simulations}.
Indeed, capability to explore arbitrary possibilities as-they-could-be constitutes the defining characteristic of alife as a tool for inquiry \citep{langton1997artificial}.
Such near-limitless freedom, however, can somewhat obscure an intrinsic limitation of most alife work: scale.
...
Although Alife systems can typically observe generational cycles at orders-of-magnitude faster rates than biological counterparts, population sizes tend to be limited, to say nothing of the profound differences in interaction richness, genetic information, and developmental richness between biological organisms and artificial life models.
...
% Adversely? Crucially? Conversely?
Aversely, scale is core to key directions in artificial life research, like transitions in individuality, ecological dynamics, and rare evolutionary innovations \citep{CITETODO}.
% Indeed, there is some evidence that limitations in scale are holding back TODO \citep{CITEGEB}.
Computational scale is also crucial within more application-oriented domains of modeling, cross-scale dynamics like being able to synthesize within-host infection dynamics with population-level epidemiological dynamics \citep{TODOCITE}.
However, because capabilities of current Silicon-based processor cores are thought unlikely to improve markedly in the forseeable future \citep{sutter2005free},
scale-up necessary to achieve progress on these key alife frontiers demands many-processor computation, which imposes fundamental compromises with respect to the convienence, flexibility, observability, interprability, and replicability enjoyed under centralized, serial models of computation. % incorporation, compromise
Here, we explore alignment of digital evolution to hardware at the extreme cutting-edge of massively-parallel accelerator-driven computation and develop algorithms to mitigate such trade-offs.

\subsection{Approaches and Progress toward Scale-up in Artificial Life}

Highly-scalable artificial life and digital evolution systems typically thread two distinct aspects of engineering effort.
As with any work in high-performance scientific computing, careful design is required to appropriately divvy computation and communication workloads across available hardware.
% flexible/pliable/creative/abstract/representational/invites
However, the discretionary nature of artificial life modeling, permits intentional formulation of simulation semantics to tailor the structure of underlying computation suitable for hardware capabilities.
Ackley's ongoing work with the T2 Tile Project and ULAM exemplifies this dichotomy \citep{CITEULAM}.
Ackley formulates simulation content in terms of local interactions between discrete particles, providing for a mapping between simulation space and hardware components and minimizing requirements for hardware connectivity.
Attention is also paid to ensure robustness of simulation to lost or corrupted state to enable relaxation of guarantees from the underlying hardware.
software infrastructure necessary to instantiate across, with additionally dipped down a level to engineer bespoke underlying hardware \citep{ILLUMINAXMACHINA,T2PROJECT}.
...
A good number of additional digital evolution systems have made progress on these two engineering fronts, enabling incorporation of rich interactions between parallelized simulation components.
explored this design space.
...
More recently, in a continuation of much earlier work, Heinemann's ALIEN project has leveraged GPU acceleration for perform physics-based simulation of numerous soft body agents within a 2D arena \citep{heinemann2008artificial}.
TODO cite conduit/dishtiny here

Beyond alife, broader realms of application-oriented evolutionary computation have explored, notably island-model and director-worker paradigms \citep{TODO}.
...
A key difference, however, exists between artificial intelligence.
...
% It is also important to be able to observe systems that are meant for artistic means, likewise.
